...
joined.A.Kssd = joined[c("fold.change.param","AFIRT.Kssd_", "AFIRT.Kssd", "AFIRT.sim", "param","drug", "isSol")]
joined.T.Kssd = joined[c("fold.change.param","TFIRT.Kssd_", "TFIRT.Kssd", "TFIRT.sim", "param","drug", "isSol")]

joined.A.Kss = joined[c("fold.change.param","AFIRT.Kss_", "AFIRT.Kss", "AFIRT.sim", "param","drug", "isSol")]
joined.T.Kss = joined[c("fold.change.param","TFIRT.Kss_", "TFIRT.Kss", "TFIRT.sim", "param","drug", "isSol")]

joined.A.Kd = joined[c("fold.change.param","AFIRT.Kd_", "AFIRT.Kd", "AFIRT.sim", "param","drug", "isSol")]
joined.T.Kd = joined[c("fold.change.param","TFIRT.Kd_", "TFIRT.Kd", "TFIRT.sim", "param","drug", "isSol")]

# Use gather to make the long data frame for ggplot. Group by isSoluble to create to separate plots.
plots.A.Kssd = joined.A.Kssd %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)
plots.T.Kssd = joined.T.Kssd %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)

plots.A.Kss = joined.A.Kss %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)
plots.T.Kss = joined.T.Kss %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)

plots.A.Kd = joined.A.Kd %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)
plots.T.Kd = joined.T.Kd %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  group_by(isSol)
...
plots.A.Kss = plots.A.Kss %>%
  mutate(param = str_replace(param,"M","T"),
         ...
         key   = str_replace(key,"_","*"),
         key   = str_replace(key,"AFIRT\\.",""),
         key   = str_replace(key,"Kss","theory.Kss"),
         key   = str_replace(key,"sim","simulation"))
...
plots.A.Kd = plots.A.Kd %>%
  mutate(param = str_replace(param,"M","T"),
         ...
         key   = str_replace(key,"_","*"),
         key   = str_replace(key,"AFIRT\\.",""),
         key   = str_replace(key,"Kd","theory.Kd"),
         key   = str_replace(key,"sim","simulation"))
...
g = g + scale.y.log10(limits=c(.001,50))
g = g + geom_line()
