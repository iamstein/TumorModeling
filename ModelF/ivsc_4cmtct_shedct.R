...
ivsc_4cmtct_shedct_Vlink = function(target = TRUE) {
  model      = ivsc_4cmtct_shedct()
  model$name = 'ivsc_4cmtct_shedct_Vlink'
  ...
  model$init      = function(p){
    ...
    ksyn = with(p,c(ksynS1,ksynS3,ksynM1,ksynM3))
    K    = with(p,matrix(c( keS1+k13S    , -k31S*V3/V1       , -kshedM1           ,  0,
                            -k13S*V1/V3 ,  keS3+k31S          ,  0                 , -kshedM3,
                            0            ,  0                  ,  kshedM1+keM1+k13M , -k31M*V3/V1,
                            0            ,  0                  , -k13M*V1/V3      ,  kshedM3+keM3+k31M), 
                         nrow = 4, byrow=TRUE))
    x    = solve(K,ksyn)
  ...
  if (target){
    ...
    D1           = AmtD1/V1                                                                                                                               ;
    d/dt(AmtD0)  =  -ka *AmtD0                                                                                                                             ;
    d/dt(AmtD1)  =(F*ka *AmtD0/V1 - k12D*D1 + k21D*V2/V1*D2      - k13D *D1  + k31D *V3 /V1* D3    - kon1*D1*(S1+M1) + koff1*(DS1+DM1) - keD1 *D1)*V1;
    d/dt(D2)     =                 - k21D*D2 + k12D*V1/V2*D1                                                                                             ;
    d/dt(D3)     =                                                  - k31D *D3  + k13D *V1 /V3* D1    - kon3*D3*(S3+M3) + koff3*(DS3+DM3) - keD3 *D3     ;
    d/dt(S1)     = ksynS1 + kshedM1*M1                              - k13S *S1  + k31S *V3 /V1* S3    - kon1*D1*S1      + koff1*DS1       - keS1 *S1     ;
    d/dt(S3)     = ksynS3 + kshedM3*M3                              - k31S *S3  + k13S *V1 /V3* S1    - kon3*D3*S3      + koff3*DS3       - keS3 *S3     ;
    d/dt(M1)     = ksynM1 - kshedM1*M1                              - k13M *M1  + k31M *V3 /V1* M3    - kon1*D1*M1      + koff1*DM1       - keM1 *M1     ;
    d/dt(M3)     = ksynM3 - kshedM3*M3                              - k31M *M3  + k13M *V1 /V3* M1    - kon3*D3*M3      + koff3*DM3       - keM3 *M3     ;
    d/dt(DS1)    =          kshedDM1*DM1                            - k13DS*DS1 + k31DS*V3/V1*DS3   + kon1*D1*S1      - koff1*DS1       - keDS1*DS1    ;
    d/dt(DS3)    =          kshedDM3*DM3                            - k31DS*DS3 + k13DS*V1/V3*DS1   + kon3*D3*S3      - koff3*DS3       - keDS3*DS3    ;
    d/dt(DM1)    =        - kshedDM1*DM1                            - k13DM*DM1 + k31DM*V3/V1*DM3   + kon1*D1*M1      - koff1*DM1       - keDM1*DM1    ;
    d/dt(DM3)    =        - kshedDM3*DM3                            - k31DM*DM3 + k13DM*V1/V3*DM1   + kon3*D3*M3      - koff3*DM3       - keDM3*DM3    ;
    '
  } else {
    model$rxode.str = '
    D1           = AmtD1/V1                                                                                                                               ;
