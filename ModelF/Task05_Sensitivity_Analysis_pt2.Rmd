...
for (i in 1:length(drugs)){
  ...
  # Set range for parameters of interest in SA.
  #---------- Add 
  params.to.iterate = data.frame(ksynM3 = lseq(df_param$ksynM3*.1, df_param$ksynM3*10, 7),
                                 k13D = lseq(df_param$k13D*.1, df_param$k13D*10, 7),
                                 dose = lseq(.1,10,7)*scale.mpk2nmol
                                 )
  dfs = list()
