...
sim = AFIRT_sim(param.as.double = p,
                model = ivsc_4cmtct_shedct(),
                dose.nmol = 0.08,
                tmax = 28*3,
                dosing.interval = 14,
...
#    print(AFIRT.theory(p=p, dose.nmol=dose.nmol, tau=14))
    row = append(c(dose.nmol), AFIRT.theory(p=p, dose.nmol=dose.nmol,tau=14))
    df = rbind(df, row)
}

colnames(df) = c("dose.nmol", "AFIRT.theory.Kss", "AFIRT.theory.Kd")
print(df)


# The following is the scatter plot the AFIRT.theory.Kss and AFIRT.theory.Kd
# Against dose.nmol. AFIRT.theory.Kss is green, and AFIRT.theory.Kd is red
{
g = ggplot(df, aes(x=dose.nmol, y=AFIRT.theory.Kss))
g = g + geom_point(color="green")
g = g + geom_point(aes(y=AFIRT.theory.Kd), color="red")
print(g)
}
