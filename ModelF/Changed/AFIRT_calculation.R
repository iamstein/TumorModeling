...
compare.thy.sim = function(model                 = model,
  ...
  # Save initial parameter vector  
  param0 = param.as.double
  param.to.change.name = param.to.change #character of parameter to
  param.to.change.0    = ifelse(param.to.change.name=="dose",
                                dose.nmol,
                                param0[param.to.change.name])
  

  
  # Simulation - iterate through values in range
  df_sim = data.frame()
  ...
  #save parameters that were changed in the simulation output
  df_sim = df_sim %>% 
    mutate(param.to.change = param.to.change.range,
           fold.change     = param.to.change.range/param.to.change.0)
  
  # Theory - Iterate through values in range.
  df_thy = data.frame()
  if (param.to.change == 'dose'){
    for (param.iter in param.to.change.range){
