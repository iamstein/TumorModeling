...
ivsc_3cmtct_shed3 = function() {
  model           = list()
  model$name      = as.character(sys.calls()[[sys.nframe()]])
  
  #COMPARTMENTS AND INITIAL CONDITIONS
  model$cmtshort  = c('AmtD0','D1','D2','D3','S1','S3','M3','DS1','DS3','DM3')
  model$init      = function(p){
             init = c(AmtD0=0,D1=0,D2=0,D3=0,S1=0,S3=0,M3=0,DS1=0,DS3=0,DM3=0)
             p    = p %>% t() %>% as.data.frame()
                
                
             ksyn = with(p,c(ksynS1,ksynS3,ksynM3))
             K    = with(p,matrix(c(keS1+k13S         ,        -k13S*VS1/VS3, 0,
                                        -k31S*VS3/VS1 ,  keS3 + k31S,        -kshedM3,
                                         0            ,  0,                   kshedM3+keM3), nrow = 3, byrow=TRUE))
             x    = solve(K,ksyn)
  ...
  model$rxode.str = '
     ...
     d/dt(S1)     = ksynS1               - k13S *S1 +     k31S*VS3/VS1*S3  - keS1 *S1  - kon1*D1*S1      + koff1*DS1;
     d/dt(S3)     = ksynS3 +kshedM3*M3   + k13S *VS1/VS3*S1     - k31S*S3  - keS3 *S3  - kon3*D3*S3      + koff3*DS3;
     d/dt(M3)     = ksynM3 -kshedM3*M3                                     - keM3 *M3  - kon3*D3*M3      + koff3*DM3;
     d/dt(DS1)    =                      - k13DS*DS1 + k31DS*VDS3/VDS1*DS3 - keDS1*DS1 + kon1*D1*S1      - koff1*DS1;
