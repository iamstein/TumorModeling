...
joined.AT = joined[c("fold.change.param","AFIRT.Kssd_", "AFIRT.Kssd","AFIRT.sim", "TFIRT.Kssd_", "TFIRT.Kssd","TFIRT.sim","param","drug", "isSol","M30","S30","Thiele")] %>%
...
plots.AT = joined.AT %>% 
  ...
  mutate(Target = ifelse(isSol,"Soluble","Membrane-Bd"),
         ...
         key    = case_when(key == "Thiele.Modulus" ~ "Thiele.Modulus",
                            str_detect(key,".sim$") ~ "Simulation",
                            str_detect(key,"Kssd_$")~ "Theory",
                            str_detect(key,"Kssd$") ~ "Theory.simple"),
         key    = factor(key,levels=c("Simulation","Theory","Theory.simple","Thiele.Modulus")),
...
g = ggplot(plots.A, aes(fold.change.param, AFIRT.value, color=key, shape=key, linetype=key,size=key))
...
width = 10
height = 6
ggsave("../results/Task09dV2_AFTIR_vs_AFTIRsimple_wThiele.pdf",width = width,height=height)
...
print(g)
ggsave("../results/Task09dV2_TFTIR_vs_TFTIRsimple_wThiele.pdf",width = width,height=height)
