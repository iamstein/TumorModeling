...
lumped.parameters.theory = function(param.as.double = param.as.double,
    ...
    numerator.DM3   = with(pars, k13DM*(V1/V3)* ksynM1                     + (keDM1 + kshedDM1 + k13DM)* ksynM3)
    numerator.M3    = with(pars, k13M *(V1/V3)* ksynM1                     + (keM1  + kshedM1  + k13M )* ksynM3)
    numerator.DM1   = with(pars, k31DM*(V3/V1)* ksynM3                     + (keDM3 + kshedDM3 + k31DM)* ksynM1)
    numerator.M1    = with(pars, k31M *(V3/V1)* ksynM3                     + (keM3  + kshedM3  + k31M )* ksynM1)
    ...
    numerator.DS3  = with(pars, k13DS*(V1/V3)*(ksynS1 + kshedDM1*M1tot.ss) + (keDS1             + k13DS)*(ksynS3 + kshedDM3*M3tot.ss))
    numerator.S3   = with(pars, k13S *(V1/V3)*(ksynS1 + kshedM1 *M10)      + (keS1              + k13S) *(ksynS3 + kshedM3 *M30))
    ...
    B = with(pars, (k13D/(keD3 + k31D) * (V1/V3)))
    Biodist = B
    
    # Clearance
    CL = with(pars, (keD1*V1))
