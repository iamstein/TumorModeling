...
# Import parameters
d = xlsx::read.xlsx("../data/ivsc_4cmtct_shedct_param.xlsx",1)
...
```
# A function that plots the sensitivity analysis for AFIRT
...
```
# A fuction that simulates the Model F
Argument:
...
# Mtot3 agaist time
```{r}
g = ggplot(df, aes(time, Mtot3)) + geom_point()
print(g)
```
...
```
# Test the theory and simulation of the following parameters
### M30, M3tot.ss, B, Davg3 
...
#    B = with(p, (k13D*VD1/VD3)/(keD3 + k31D))
    ...
    lumped_parameters_theory = c(M30, Mtot3.ss, Tacc.tum)
...
```
## Calculate lumped parameters from simulation for Atezolizumab
Different dose should give same result. This can used as a sanity check. I will run simulation for dose.nmol=80 and dose.nmol=8000
to see if they give the same results
```{r}
sim_80 = simulation(dose.nmol=80, "../data/ModelF_Atezolizumab_Params.xlsx")
initial_state = sim_80 %>%
    filter(time==0)
M30_sim_80 = initial_state$M3

sim_8000 = simulation(dose.nmol=8000, "../data/ModelF_Atezolizumab_Params.xlsx")
initial_state = sim_8000 %>%
    filter(time==0)
M30_sim_8000 = initial_state$M3
print(c(M30_sim_80, M30_sim_8000))
```
Awesome, they do agree.
...
lumped.parameters.simulation = function(dose.nmol, params_file_path){
  ...
  lumped_parameters_sim = c(M30, Mtot3.ss, Tacc.tum)
  names(lumped_parameters_sim) = c("M30", "Mtot3.ss", "Target Accumulation Rate")
