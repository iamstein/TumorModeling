...
joined.A = joined[c("fold.change.param","AFIRT.Kssd", "AFIRT.Kss", "AFIRT.Kd", "AFIRT.sim", "param","drug", "isSol")]

# Use gather to make the long data frame for ggplot.
plots.A = joined.A %>% gather(key, AFIRT.value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(AFIRT.value)) %>%
  # Group by isSoluble to create to separate plots.
  group_by(isSol) %>%
  do(plot = ggplot(., aes(fold.change.param, AFIRT.value, color=key)) +
      scale.x.log10() +
      scale.y.log10() +
      geom_line() +
      geom_point() + 
...
```{r}
# I only wanted a subset of the parameters for gather.
joined.T = joined[c("fold.change.param","TFIRT.Kssd", "TFIRT.Kss", "TFIRT.Kd", "TFIRT.sim", "param","drug", "isSol")]

# Use gather to make the long data frame for ggplot.
plots.T = joined.T %>% gather(key, TFIRT.value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(TFIRT.value)) %>%
  # Group by isSoluble to create to separate plots.
  group_by(isSol)%>%
  do(plot = ggplot(., aes(fold.change.param, TFIRT.value, color=key, linetype=key)) +
      scale.x.log10() +
      scale.y.log10() +
      geom_line() +
      geom_point() + 
