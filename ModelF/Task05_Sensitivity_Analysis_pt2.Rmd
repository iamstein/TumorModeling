...
for (i in 1:length(drugs)){
  ...
  # Set range for parameters of interest in SA.
  # Check which parameters are nonzero, not including dose which isn't int df_param.
  nnzero = df_param[names(parameters)[which(parameters != "dose")]] != 0
...
plots$plot
```
>>>>>>> Updated plotting section to split into separate plots according to solubility of drug target.
