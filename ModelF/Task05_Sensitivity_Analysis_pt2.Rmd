...
plots.T = joined.T %>% gather(key, TFIRT.value, -param, -fold.change,-drug, -isSol) %>%
  ...
  # Group by isSoluble to create to separate plots.
  group_by(isSol)%>%
  do(plot = ggplot(., aes(fold.change, TFIRT.value, color=key, linetype=key)) +
      scale.x.log10() +
      scale.y.log10() +
      geom_line() +
      geom_point() + 
...
# I only wanted a subset of the parameters for gather.
joined.Cmin = joined[c("fold.change","Cmin.thy", "Cmin.sim", "param","drug", "isSol")]
...
plots.Cmin = joined.Cmin %>% gather(key, Cmin.value, -param, -fold.change,-drug, -isSol) %>%
  ...
  # Group by isSoluble to create to separate plots.
  group_by(isSol)%>%
  do(plot = ggplot(., aes(fold.change, Cmin.value, color=key,linetype=key)) +
      scale.x.log10() +
      scale.y.log10() +
      geom_line() +
      geom_point() + 
