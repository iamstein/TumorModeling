...
# drugs - which drug we are performing SA on. This is just to create a title for the graph.
...
compare.thy.sim = function(model                 = model, 
  ...
  g = ggplot(Data, aes(Param, AFIRT.value, color=key)) +
      ...
      geom_line() +
      geom_point() +
      labs(x = param.to.change, title = drugs)
  print(g)
...
drugs           = c("Atezolizumab", "Herceptin", "Pembrolizumab", "Bevacizumab"); paths = NULL; i = 4
...
# ksynS3.range    = lseq(df_param$ksynS3  *.1, df_param$ksynS3  *10, 7)
# kshedS3.range   = lseq(df_param$kshedS3 *.1, df_param$kshedS3 *10, 7)
# kshedDS3.range  = lseq(df_param$kshedDS3*.1, df_param$kshedDS3*10, 7)
...
keM3.range      = lseq(df_param$keM3    *.1, df_param$keM3    *10, 7)
keDM3.range     = lseq(df_param$keDM3   *.1, df_param$keDM3   *10, 7)
keS3.range      = lseq(df_param$keS3    *.1, df_param$keS3    *10, 7)
keDS3.range     = lseq(df_param$keDS3   *.1, df_param$keDS3   *10, 7)
...
df_compare = compare.thy.sim(model = model,
                             ...
                             tau  = tau,
                             compartment = compartment,
                             param.to.change       = 'keDS3',
                             param.to.change.range = keDS3.range,
                             soluble               = isSoluble)
```
