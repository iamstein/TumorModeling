...
##################### User input #####################
# Variable of interest. Options: 'AFIRT', 'TFIRT' 
var.of.int = 'TFIRT' 
##################### User input #####################

var.of.int1 = paste(var.of.int,'.Kssd',sep='')
var.of.int2 = paste(var.of.int,'.Kss' ,sep='')
var.of.int3 = paste(var.of.int,'.Kd'  ,sep='')
var.of.int4 = paste(var.of.int,'.sim' ,sep='')

# I only wanted a subset of the parameters for gather.
joined.plot = joined[c("fold.change.param", var.of.int1, var.of.int2, var.of.int3, var.of.int4, "param", "drug", "isSol")]

# Use gather to make the long data frame for ggplot.
plot.all = joined.plot %>% gather(key, Value, -param, -fold.change.param, -drug, -isSol) %>%
  filter(!is.na(Value)) %>%
  # Group by isSoluble to create to separate plots.
  group_by(isSol) %>%
  do(plot = ggplot(., aes(fold.change.param, Value, color=key)) +
     scale.x.log10() +
     scale.y.log10() +
     geom_line() +
     geom_point() + 
